...
if [ -n "${INPUT_MARKDOWN_FILE}" ]; then
  FILE_VERSION=$(head -20 "${INPUT_MARKDOWN_FILE}" | grep -E "version: |subtitle: Version " | sed -E 's/version: |subtitle: Version /v/')
...
DIFF_COMMIT=
if [ -n "${DIFF_FILE}" ]; then
  DIFF_VERSION=$(head -20 "${DIFF_FILE}" | grep -E "version: |subtitle: Version " | sed -E 's/version: |subtitle: Version /v/')
